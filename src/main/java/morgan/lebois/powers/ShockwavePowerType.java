...
public class ShockwavePowerType extends PowerType {
    ...
    public @NotNull PowerConfiguration<?> getConfig() {
        return LeboisPowerTypes.SHOCKWAVE;
    }
    ...
    public void triggerShockwave(ServerPlayerEntity player, Vec3d velocity, Vec3d pos) {
        ...
        world.playSound(null, pos.x, pos.y, pos.z, LeboisSounds.SONIC_BOOM, SoundCategory.PLAYERS, 3.0F, 1.0F);

        entities.forEach(entity -> {
            if ( entity instanceof LivingEntity) {
                if (world.getRandom().nextFloat() < 0.05F) {
                    Util.sendMovingSound(LeboisSounds.PATRICK_SCREAM, 2.3F, 1.0F, entity, world.getRandom().nextLong());
                }
            }
