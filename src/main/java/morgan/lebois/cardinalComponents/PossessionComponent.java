...
import morgan.lebois.mixin.common.client.world.ClientWorldAccessor;
...
public class PossessionComponent implements AutoSyncedComponent {
    ...
    private static MobEntity findEntityByUuid(World world, UUID uuid) {
        ...
        if ( world instanceof ServerWorld serverWorld ) {
            ...
            possessedEntity = ((ClientWorldAccessor) clientWorld).lebois$getEntityLookup().get(uuid);
        }
    ...
    public void setPossessedEntity(@Nullable MobEntity entity) {
        ...
        if ( this.possessedEntityUuid != uuid ) {
            ...
            this.possessedEntity = entity;
            LeboisEntityComponents.POSSESSION.sync(this.playerEntity);
        }
    }

    @Override
    public void readFromNbt(NbtCompound tag, RegistryWrapper.WrapperLookup registryLookup) {
        if (tag.containsUuid(Lebois.stringId("possession"))) {
            this.possessedEntityUuid = tag.getUuid(Lebois.stringId("possession"));
    ...
    public void writeToNbt(NbtCompound tag, RegistryWrapper.WrapperLookup registryLookup) {
        if( this.possessedEntityUuid != null ) {
            tag.putUuid(Lebois.stringId("possession"), this.possessedEntityUuid);
        }
    }
}