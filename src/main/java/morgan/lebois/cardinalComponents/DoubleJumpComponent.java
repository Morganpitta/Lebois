...
public class DoubleJumpComponent implements AutoSyncedComponent {
    ...
    public void setDoubleJumps(int doubleJumps) {
        if (this.doubleJumps != doubleJumps) {
            ...
            LeboisEntityComponents.DOUBLE_JUMP.sync(this.livingEntity);
        }
    }

    @Override
    public void readFromNbt(NbtCompound tag, RegistryWrapper.WrapperLookup registryLookup) {
        doubleJumps = tag.getInt(Lebois.stringId("double_jumps"));
    }

    @Override
    public void writeToNbt(NbtCompound tag, RegistryWrapper.WrapperLookup registryLookup) {
        tag.putInt(Lebois.stringId("double_jumps"), doubleJumps);
    }
}