...
public class LeboisClientPackets {
    ...
    private static void tryPossess(PlayerEntity player, int entityId, int retries) {
        ...
        if (entity instanceof MobEntity mob) {
            ((PossessionInterface) player).lebois$possess(mob);
        }
        else {
            if (retries > 0) {
                ...
                Lebois.LOGGER.warn("Failed to possess entity ID {}, entity was never found", entityId);
            }
        }
    }
