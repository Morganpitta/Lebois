...
import morgan.lebois.powers.ActionOnCoinPowerType;
import morgan.lebois.powers.ActionOnKeyReleasePowerType;
import morgan.lebois.powers.FrostGlidingPowerType;
import morgan.lebois.powers.ShockwavePowerType;
...
public class LeboisPackets {
    public static void register() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(CoinHitC2SPacket.ID, LeboisPackets::handleUseCoinPowerType);
        ServerPlayNetworking.registerGlobalReceiver(FrostGlideC2SPacket.ID, LeboisPackets::handleFrostGlide);
        ServerPlayNetworking.registerGlobalReceiver(ShockwavePowerTypesC2SPacket.ID, LeboisPackets::handleShockwavePacket);
        ServerPlayNetworking.registerGlobalReceiver(FlyingC2SPacket.ID, LeboisPackets::handleFlyingPacket);
    }
    ...
    public static void handleUseKeyReleasePowerType(UseKeyReleasePowerTypesC2SPacket payload, ServerPlayNetworking.Context context) {
        ...
        for (Identifier powerId : payload.powerIds()) {
            ...
            if (powerType instanceof ActionOnKeyReleasePowerType keyReleasePowerType) {
                ...
                Lebois.LOGGER.warn("Unexpectedly found power \"{}\" (which doesn't have a key release power type) while receiving packet for triggering active power types of player {}!", powerId, player.getName().getString());
            }
            else {
                Lebois.LOGGER.warn("Found unknown power \"{}\" while receiving packet for triggering key release power types of player {}!", powerId, player.getName().getString());
            }
        }
    }
    ...
    public static void handleFrostGlide(FrostGlideC2SPacket payload, ServerPlayNetworking.Context context) {
        ...
        server.execute(() -> {
            ...
            BlockState frostBlockState = LeboisBlocks.FROST_BLOCK.getDefaultState();
    ...
    public static void handleShockwavePacket(ShockwavePowerTypesC2SPacket payload, ServerPlayNetworking.Context context) {
        ...
        for (Identifier powerId : payload.powerIds()) {
            ...
            if (powerType instanceof ShockwavePowerType shockwavePowerType) {
                ...
                Lebois.LOGGER.warn("Unexpectedly found power \"{}\" (which doesn't have a shockwave power type) while receiving packet for triggering active power types of player {}!", powerId, player.getName().getString());
            }
            else {
                Lebois.LOGGER.warn("Found unknown power \"{}\" while receiving packet for triggering shockwave power types of player {}!", powerId, player.getName().getString());
            }
        }
    }
    ...
    public static void handleFlyingPacket(FlyingC2SPacket payload, ServerPlayNetworking.Context context) {
        ...
        server.execute(() -> {
            ((Winged) context.player()).lebois$setFlying(payload.isFlying());
        });
    }
}
